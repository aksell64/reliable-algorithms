...
func (r *Registry) Register(name string, factory func() any) {
	...
	if r.debug {
		log.Printf("registered type %T for name %q", resolve(factory()), name)
	}
}
...
func (r *Registry) Unmarshal(b []byte, name string) (any, error) {
	...
	if m, ok := ptr.(Unmarshaler); ok {
		if r.debug {
			log.Printf("unmarshaling type %T (%s) using custom Unmarshaler", resolve(ptr), name)
		}
	...
	if r.debug {
		log.Printf("unmarshaling type %T (%q) using default unmarshaler", resolve(ptr), name)
	}
